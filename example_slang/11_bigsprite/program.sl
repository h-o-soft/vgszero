#include VGS0.SL
#include palette.sl

CONST SPRITE_NUM = 256;

// グローバル変数
//typedef struct {
//    // スプライトの表示情報
//    struct Obj {
//        int8_t flag;
//        int8_t big;
//        int8_t x;
//        int8_t y;
//        uint8_t an;
//        uint8_t pi;
//    } obj[SPRITE_NUM];
//} GlobalVariables;
//#define GV ((GlobalVariables*)0xC000)
// 構造体は無い……
ARRAY BYTE obj[SPRITE_NUM * 6];
CONST Obj_flag = 0;
CONST Obj_big = 1;
CONST Obj_x = 2;
CONST Obj_y = 3;
CONST Obj_an = 4;
CONST Obj_pi = 5;
CONST Obj_MAX = 6;

// image00 のパターン番号
CONST cptnnum = {
    0x00, 0x03, 0x06, 0x09, 0x0C, 0x30, 0x33, 0x36, 0x39, 0x3C};
ARRAY BYTE ptnnum[]:cptnnum;

// 乱数テーブル
CONST crandom = {
    };
ARRAY BYTE random[]:crandom;

main()
VAR BYTE n;
VAR BYTE r;
VAR BYTE i;
VAR idx;
{
    // パレットを初期化
    init_palette();

    // Bank 2 を Sprite パターンとして利用
    VGS0_ADDR_SPRITE_DPM = 2;

    // グローバル変数を初期化
    vgs0_memset(obj, 0, SPRITE_NUM * 6 /* sizeof(GlobalVariables) */);

    // メインループ
    n = 0;
    r = 0;
    loop {
        // V-BLANK を待機
        vgs0_wait_vsync();

        // 爆発を追加
        idx = n * Obj_MAX;
        if (0 == obj[idx + Obj_flag]) {
            obj[idx+ Obj_flag] = 1;
            if (n AND 7) {
                obj[idx + Obj_big] = 0;
            } else {
                obj[idx + Obj_big] = 1;
            }
            obj[idx + Obj_an] = 0;
            obj[idx + Obj_pi] = 0;
            obj[idx + Obj_x] = random[r];
            r++;
            obj[idx + Obj_y] = random[r] MOD 192;
            r++;
            if (obj[idx + Obj_big]) {
                vgs0_oam_set(n, obj[idx + Obj_x], obj[idx + Obj_y], 0x80 + ((n AND 1) * 0x20), 0, 11, 11);
                VGS0_ADDR_OAM[n*8 + OAM_BANK] = 3;
            } else {
                vgs0_oam_set(n, obj[idx + Obj_x], obj[idx + Obj_y], 0x80 + ((n AND 1) * 0x20), ptnnum[0], 2, 2);
            }
            n++;
        }

        // スプライトを4フレームに1回アニメーション
        i = 0;
        repeat {
            idx = i*Obj_MAX;
            if (obj[idx + Obj_flag]) {
                obj[idx + Obj_an]++;
                obj[idx + Obj_an] = obj[idx + Obj_an] AND 0x03;
                if (0 == obj[idx + Obj_an]) {
                    if (obj[idx + Obj_big]) {
                        // 12x12 の超スプライトはバンク切り替えでアニメーション
                        VGS0_ADDR_OAM[i*8 + OAM_BANK] += 1;
                        if (17 < VGS0_ADDR_OAM[i*8 + OAM_BANK]) {
                            // 削除
                            obj[idx + Obj_flag] = 0;
                            VGS0_ADDR_OAM[i*8 + OAM_ATTR] = 0;
                        }
                    } else {
                        // 3x3 の巨大スプライトはパターン切り替えでアニメーション
                        obj[idx + Obj_pi]++;
                        if (obj[idx + Obj_pi] < 10) {
                            VGS0_ADDR_OAM[i*8 + OAM_PTN] = ptnnum[obj[idx + Obj_pi]];
                        } else {
                            // 削除
                            obj[idx + Obj_flag] = 0;
                            VGS0_ADDR_OAM[i*8 + OAM_ATTR] = 0;
                        }
                    }
                }
            }
            i++;
        } until (not i);
    }
}